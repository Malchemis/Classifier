...
def remove_NaN_filename(csv_file):
    ...
    """
    initial_len = len(csv_file)
    csv_file  = csv_file.dropna(subset=['filepath_wav_20'])
    print(f'Removed {initial_len - len(csv_file)} rows where the filename is NaN')
    return csv_file
...
def remove_0_vehicle_class(csv_file):
    ...
    print(f'Removed {initial_len - len(csv_file)} rows where the vehicle_class is 0') 
    return csv_file
...
def delete_files_from_folders(csv_file, basedir): 
    ...
    wav_files_to_delete = [x for x in wav_files if x not in csv_file["filepath_wav_20"].tolist()]
    ...
    csv_rows_to_delete = csv_file[~csv_file["filepath_wav_20"].isin(wav_files_to_keep)]
    # Get the csv row that should not be deleted
    csv_rows_to_keep = csv_file[~csv_file["filepath_wav_20"].isin(csv_rows_to_delete["filepath_wav_20"].tolist())]
...
def delete_empty_files(path_csv, basedir): 
    ...
    for filename in csv_file['filepath_wav_20'].unique(): 
    ...
    if len(empty_files) != 0:
        ...
        new_csv = csv_file[~csv_file['filepath_wav_20'].isin(empty_files)]
        # Rewrite the csv
...
def delete_short_files(path_csv, basedir, min_duration=17): 
    short_files = []
    csv_file = pd.read_csv(path_csv, sep='\t')  
    for filename in csv_file['filepath_wav_20'].unique(): 
    ...
    if len(short_files) != 0:
        ...
        new_csv = csv_file[~csv_file['filepath_wav_20'].isin(short_files)]
        # Rewrite the csv
        new_csv.to_csv(path_csv, sep='\t', index=False)
